-- Recordatorios de recurrentes manuales — CLAUDE.md § recurrentes: con
-- auto-registro OFF la fecha de la regla es aviso/organización, no un hecho
-- contable, así que el usuario necesita que algo se lo recuerde en tres
-- momentos: el día previo, el día exacto, y el día posterior si todavía no
-- lo cargó. El aviso reactivo que ya existía ("se materializó hoy") es
-- para auto-registro ON — este es el que faltaba para OFF.
--
-- Reescribe `dispatch_due_notifications()` completo (no hay forma de
-- agregar un bloque a una función existente sin reemplazarla entera) para
-- sumar un nuevo loop, inmediatamente después del de `recurring_reminders`
-- reactivo. Mismo `kind` de push (`recurring_reminders`) — es la misma
-- preferencia de usuario ("recordatorios de recurrentes"), solo cambia el
-- mensaje y la clave de dedup (`recurring_manual_reminder` + `occurrence_date`
-- + `phase`, para que las tres fases de una misma ocurrencia avisen cada
-- una una sola vez).

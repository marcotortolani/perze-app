...
**`kind → signo de flujo` sale de `src/lib/analytics/cash-flow.ts`, nunca inline.** Comprar o vender un instrumento (`kind = 'investing'`) cuenta como flujo de caja (Ingresos/Egresos del período, flujo de dinero, patrimonio neto) pero no como consumo (presupuestos, gasto por categoría, mapa de calor) — son dos preguntas distintas sobre el mismo `kind`, y `classifyCashFlow`/`classifyConsumption` son las únicas dos funciones que las responden. Antes de este helper hubo doce reimplementaciones inline del mismo `if (kind === "expense") ... else ...`, y tres de ellas tenían el signo mal — una compra restando de los ingresos, otra contada como gasto invertido. Un agregado nuevo sobre `transactions` que necesite saber "¿esto entró o salió?" pasa por acá, no por un switch propio.

...
- **`fit`:** encoge el `font-size` hasta `FIT_FLOOR` (55% del tamaño nominal) cuando el contenedor no alcanza — un patrimonio de varios millones no entra a `hero` 40px en un teléfono angosto, ni un monto de muchos dígitos entra a `hero-xl` 64px en el keypad de captura. Mide con `ResizeObserver` y escala `font-size`/`line-height` en vez de `transform: scale()`, para que la caja de layout y la nitidez del texto sigan correctas. El contenedor tiene que darle un ancho definido (bloque con `width` fija bajo un padre acotado, nunca `inline-flex`/ancho intrínseco) — si no, no hay nada contra qué encoger. `AmountScrubber` (C1) lo usa con `style={{ width: "90%" }}`: ese 10% de margen fijo a los costados es el padding pedido, no algo que `fit` calcule solo — su propio contenedor pasó de `inline-flex` a `flex` + `width: 100%` para poder medir.

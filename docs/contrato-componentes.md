...
- **Nota:** entra y sale con animación real (240ms, `--ease-spring-snappy`; 0 con intensidad "mínima") — `Overlay` desmonta recién cuando termina la transición de salida, no en el mismo frame en que `open` pasa a `false`. Un cierre por arrastre que llega al umbral continúa el mismo recorrido en vez de saltar a 0 antes de irse. Esto no aplica a los sheets que se cierran vía cambio de `key` (remount forzado, p. ej. `EditCategorySheet`) — ahí React desmonta de inmediato, sin animación de salida.
...
- **Estados:** positivo/negativo/negativo-énfasis/neutro · con flecha · con decimales atenuados (keypad) · privacidad activa (la cifra real sigue en el DOM con `visibility: hidden` — define la caja, cero salto de layout al prender/apagar — cubierta por una píldora con blur + degradé del mismo ancho/alto exacto; el blur cae sobre esa forma propia, nunca sobre los glifos, así no depende de sangría ni se recorta contra un ancestro con `overflow: hidden`) · `fit` (encogido).

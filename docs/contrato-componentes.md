...
- **Props:** `value: Money` · `size?: 'hero-xl'|'hero'|'title'|'body'|'label'` (default `body`) · `polarity?: 'positive'|'negative'|'negative-emphasis'|'neutral'` (default derivado del signo, `neutral` si `showSign=false`) · `showSign?: boolean` (default `true`) · `showArrow?: boolean` · `tabular?: boolean` (mono + `tabular-nums`, solo en columnas) · `mutedDecimals?: boolean` · `privacy?: boolean` · `fit?: boolean` (default `false`).
...
- **`fit`:** encoge el `font-size` hasta `FIT_FLOOR` (55% del tamaño nominal) cuando el contenedor no alcanza — un patrimonio de varios millones no entra a `hero` 40px en un teléfono angosto. Mide con `ResizeObserver` y escala `font-size`/`line-height` en vez de `transform: scale()`, para que la caja de layout y la nitidez del texto sigan correctas. El contenedor tiene que darle un ancho definido (bloque con `width: 100%` bajo un padre acotado, nunca `inline-flex`/ancho intrínseco) — si no, no hay nada contra qué encoger. Pensado para las cifras héroe de ancho de pantalla (patrimonio, saldo, total); no se activó en `AmountScrubber` porque su contenedor es `inline-flex` para el gesto de arrastre y cambiar eso es un rediseño de layout aparte, no este arreglo.

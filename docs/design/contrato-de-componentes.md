...
## 2 bis · Las veinte que faltaban

Las fichas de abajo (sección 8) completan el contrato: el delta v2 documentaba 27 piezas
nuevas y no las 20 que las pantallas ya usaban, incluidos los cinco componentes más
instanciados de toda la app. **La implementación de referencia completa es
`library/perze-design-system.jsx`** — un solo archivo, sin imports fuera de React.
`library/perze-v2.jsx` queda obsoleto: importaba de un `./core` inexistente y no corría.
...
## 8 · Las veinte que faltaban

Mismo formato que las secciones 2 y 3. Cierran las dos ambigüedades abiertas: la
relación **FxEditor / Rate / PriceStatus** y el destino de **Sparkline**, escritas
en cada ficha involucrada.

### 8.0 · Decisión: FxEditor, Rate y PriceStatus son TRES cosas

No se solapan una vez que se nombra el eje correcto, que no es "tipo de cambio" sino
**qué hace cada uno con el valor**:

| | Qué hace | Quién lo usa |
|---|---|---|
| **Rate** | **Lee.** Muestra un tipo de cambio ya decidido. | C4, E6, E7, H6, I2 |
| **FxEditor** | **Escribe.** Produce el valor: la cifra es el objeto que se edita. | C4 (gasto en otra moneda), K8 |
| **PriceStatus** | **Ni una ni otra.** Dice de dónde salió y hace cuánto. | I2, I3, I4, I12, y adentro de los otros dos |

**PriceStatus es el átomo y los otros dos lo componen.** Hoy `FxEditor` recibe
`source`, `ageHours` y `stale` y arma su propio `StatusBadge` inline, y `Rate`
haría lo mismo: son dos implementaciones de "esto está viejo" que van a divergir. Con
la composición, `stale` deja de ser una prop booleana suelta y pasa a ser el
`state` de PriceStatus, que tiene cuatro valores y no dos — `manual` y
`market-closed` no son "stale" y hoy no tienen dónde vivir.

Consecuencia concreta para el port: **`FxEditor` pierde la prop `stale`** (queda
`state`), y **hereda la regla de precisión de Rate**. El bundle hardcodea 2 decimales
y eso rompe el par ARS/UYU, que necesita 4 — un usuario que cargue 0,03 en vez de
0,0284 se desvía 5% en cada conversión.

### 8.1 · Decisión: Sparkline se queda, con el alcance recortado

El contrato v2 lo declaró "placeholder a reemplazar por LineChart". Estaba mal: el
diseño lo usa nueve veces y en siete es correcto. La regla que faltaba no es de
tamaño, es de **contenido**:

- **Sparkline** = glifo de tendencia DENTRO de otro elemento. Sin ejes, sin grilla, sin
  labels, sin interacción, **una sola serie**, ≤120 px. No responde una pregunta: adorna
  con dato una fila que ya la responde.
- **LineChart** = el gráfico. Vive en un `ChartCard`, tiene grilla, labels directos,
  tooltip táctil y toggle de tabla. **Responde una pregunta.**

De los nueve usos: **siete se quedan** (dentro de `InsightCard` en B, D y G1; dentro de
filas de I3; en el tile de I2). **Dos migran**, y son los dos de F2 —el acumulado del mes
contra la línea ideal—, porque ahí hay **dos series superpuestas**, y dos series obligan a
leyenda, y una leyenda obliga a `ChartCard`, que obliga a toggle de tabla. Lo que cambia
en esas dos: dejan de ser dos `<Sparkline>` apilados con `position:absolute` y pasan a
ser un `LineChart series={[real, ideal]}` con la ideal en `dashed`, envuelto en
`ChartCard` con `SeriesLegend`. Es exactamente el defecto que la auditoría marcó como el
peor de F2.

---

### Button
- **Existe para:** ser la acción. `primary` es el único componente que usa relleno de marca por acción, y sólo puede haber uno visible por pantalla.
- **Props:** `variant?: 'primary'|'secondary'|'ghost'|'destructive'` · `size?: 'sm'|'md'|'lg'` (36/48/64) · `fullWidth?: boolean` (default true) · `icon?: IconName` · `disabled?: boolean` · `onClick?: () => void` · `children: ReactNode`.
- **Estados:** normal · presionado (`scale(.96)` + haptic 8 ms) · deshabilitado (opacidad .4, sin press) · foco visible (outline 2 px violeta, offset 2).
- **Tokens:** `--primary-fill --primary-on-fill --surface-3 --text-primary --text-secondary --critical --radius-button --press-scale --duration-micro --ease-spring-snappy`.
- **A11y:** `<button type="button">` real. Con `icon` y sin `children` requiere `aria-label`. Mínimo 44 px incluso en `sm` (36 de alto visible, 44 de target vía padding del contenedor). Toda acción primaria vive en los últimos 200 px de la pantalla.
- **Bloques:** todos.

### AppHeader
- **Existe para:** el chrome superior de 56 px: título o alcance, estado de sync y buscador.
- **Props:** `title?: string` · `scope?: 'Personal'|'Compartido'|'Todo'` · `onScopeChange?: (v) => void` · `scopeOptions?: string[]` · `onBack?: () => void` · `onSearch?: () => void` · `syncState?: 'synced'|'syncing'|'offline'` · `pending?: number` · `showScope?: boolean` · `right?: ReactNode`.
- **Estados:** con título · con scope (excluyentes) · con back · con buscador · colapsado al scrollear.
- **Tokens:** `--screen-padding --text-primary --text-secondary --font-sans`.
- **A11y:** `<header>` con un solo `<h1>` por pantalla. Back y buscar son botones de 44×44 con `aria-label`. El header es chrome: **no** entra en el presupuesto de 5 interactivos.
- **Bloques:** todos menos las pantallas de onboarding y L6.
- **Nota:** monta el `SegmentedControl emphasis="brand"` que reemplazó al `ScopeSwitcher` eliminado. Es el único control de alcance del sistema: ninguna pantalla agrega otro.

### Amount
- **Existe para:** una cifra de dinero, con la regla de polaridad del sistema adentro.
- **Props:** `value: number` · `currency?: CurrencyCode` · `decimals?: number` · `size?: 'hero-xl'|'hero'|'title'|'body'|'label'` · `showSign?: boolean` · `polarity?: 'positive'|'negative'|'negative-emphasis'|'neutral'` · `mutedDecimals?: boolean` · `count?: boolean`.
- **Estados:** positivo (aqua, con `+`) · negativo (tinta, con `−`) · negativo enfatizado (naranja) · neutro · con decimales atenuados · **conteo** (`count`, sin símbolo y sin decimales).
- **Tokens:** `--font-mono --money-positive --money-negative-emphasis --text-primary --text-muted --text-hero-*/--text-title-*`.
- **A11y:** `tabular-nums` siempre. El signo es un carácter real (`+`/`−`), no color: el lector de pantalla lo lee. **Nunca verde/rojo como polaridad.**
- **Bloques:** todos.
- **Cambio:** `count` es nuevo. No existía forma de renderizar un número sin moneda, y las pantallas terminaban pasando `currency="none"`, que el componente imprimía literalmente como símbolo (`none 340,00`). Bug real encontrado en K9.
- **Regla del saldo, más estricta que la spec original:** el aqua marca la polaridad de un **movimiento**, nunca la de un **saldo**. Patrimonio, saldos de cuenta y KPI van en tinta y sin signo — `showSign={false}` ya lo hace solo.

### Icon
- **Existe para:** el set de línea. Ninguna pantalla dibuja un SVG propio.
- **Props:** `name: IconName` · `size?: number` (13/19-20/22/26/28/32 son los canónicos) · `color?: string` · `strokeWidth?: number` (1.5 default; 2.5 en badges de 13, 1.25 en 32).
- **Estados:** por color y tamaño; sin estados propios.
- **Tokens:** hereda `currentColor`; por defecto `--text-secondary`.
- **A11y:** `aria-hidden="true"` y `focusable="false"` **siempre**: el ícono nunca es la etiqueta, la etiqueta la pone el contenedor. **Regla dura: cero íconos decorativos** — todo ícono es tocable o porta significado.
- **Bloques:** todos.
- **Cambio:** seis glifos nuevos — `mail`, `lock`, `fingerprint`, `install`, `globe` y `bank-checking`. El último porque caja de ahorro y cuenta corriente compartían `bank` y se veían idénticas en la lista de cuentas.

### Chip
- **Existe para:** un filtro, un atajo o una acción secundaria en línea.
- **Props:** `selected?: boolean` · `icon?: IconName` · `disabled?: boolean` · `onClick?: () => void` · `children: ReactNode`.
- **Estados:** normal · seleccionado · presionado · deshabilitado.
- **Tokens:** `--surface-2 --primary-fill --primary-on-fill --text-secondary --radius-chip --press-scale`.
- **A11y:** `aria-pressed` cuando es un toggle. 36 de alto visible, 44 de target.
- **Bloques:** B, C, D, F2, F6, G2, H (todos los toggles de tabla), I, J, K.
- **Frontera con la selección por superficie:** `selected` usa relleno de marca porque un **filtro activo** es identidad de filtro. Una **elección de configuración** (frecuencia, cuotas, clase de activo) NO es un filtro: va con pills por superficie o `SelectableRow`. Esta frontera no estaba escrita y produjo dos tratamientos opuestos entre D2 e I1.

### Card
- **Existe para:** agrupar contenido en una superficie.
- **Props:** `surface?: 1|2|3` · `padding?: number` (20 default) · `children: ReactNode`.
- **Estados:** sin estados. **Sin sombra y sin borde**, siempre.
- **Tokens:** `--surface-1..3 --radius-card`.
- **A11y:** es un contenedor de presentación; si agrupa una región con nombre, el consumidor pone `role="region"` y `aria-label`. Nunca más de 3 superficies apiladas.
- **Bloques:** todos.

### SegmentedControl
- **Existe para:** 2 a 4 opciones excluyentes visibles a la vez.
- **Props:** `options: string[] | {value,label}[]` · `value: string` · `onChange: (v) => void` · `emphasis?: 'surface'|'brand'` · `size?: 'sm'|'md'` · `label: string`.
- **Estados:** cada opción activa/inactiva · foco por teclado (flechas mueven la selección).
- **Tokens:** `--surface-2 --surface-3 --primary-fill --primary-on-fill --text-secondary --radius-chip`.
- **A11y:** `role="radiogroup"` con `aria-label` obligatorio; cada botón `role="radio"` + `aria-checked`. Segmento mínimo 44 px de ancho: con 4 opciones eso son 176 px, así que en 390 entra.
- **Bloques:** B1, C4, D2, F3, G3, H2, H3, I3, I10, J6, J8, K5, K6, K9, K10.
- **Nota:** `emphasis="brand"` es **exclusivo del scope de datos**. Cualquier otro uso rompe la regla de un violeta por pantalla.

### SkeletonRow
- **Existe para:** el placeholder de una fila de lista, con su forma exacta.
- **Props:** ninguna (la fila del sistema tiene una sola forma).
- **Estados:** pulso · estático (con intensidad de animación en Mínima).
- **Tokens:** `--surface-2`; animación `ds-pulse` 1,4 s.
- **A11y:** `aria-hidden="true"`; el contenedor lleva `aria-busy="true"`. Sin texto.
- **Bloques:** todos los estados de carga (los 26 del proyecto).
- **Cambio:** ver `Skeleton` — normalización de tamaños. Y `SkeletonBlock`, que compone las cuatro plantillas de layout, es nuevo.

### Switch
- **Existe para:** un binario que se aplica al instante, sin guardar.
- **Props:** `checked: boolean` · `onChange: (next: boolean) => void` · `disabled?: boolean` · `label: string`.
- **Estados:** apagado · encendido · deshabilitado · en transición (180 ms).
- **Tokens:** `--primary-fill --primary-on-fill --surface-3 --duration-fast --ease-spring-snappy`.
- **A11y:** `role="switch"` + `aria-checked` + `aria-label`. Target 52×32 visible, 44 mínimo garantizado.
- **Bloques:** F0, F0m, F3, G0r, G2, G3, I1, J1, J3, K2, K4, K10, K11, K12.
- **Excepción declarada, ampliada:** el encendido usa relleno de marca. El sistema la declaraba para *una* pantalla (Apariencia) y en la práctica aplica a **catorce**. La regla correcta, y la que rige: *una pantalla cuyo contenido son switches no lleva además una acción primaria*. K4, K11 y K12 la cumplen; F0, F0m, G0r, I1, J1 y J3 tienen un switch **más** un primario, y ahí el switch es el estado de activación del módulo, no un ajuste — excepción acotada a las pantallas de activación.

### Input
- **Existe para:** texto libre. **Nunca** para montos.
- **Props:** `label?: string` · `value?: string` · `defaultValue?: string` · `placeholder?: string` · `onChange?: (v) => void` · `invalid?: boolean` · `hint?: string` · `disabled?: boolean` · `type?: 'text'|'email'|'search'`.
- **Estados:** vacío · con valor · foco · inválido · deshabilitado · con hint.
- **Tokens:** `--surface-3 --radius-input --text-primary --text-muted --critical`. Superficie 3 y radio 14: **un input sí lleva borde, un card no**.
- **A11y:** `<label htmlFor>` real, `aria-invalid`, `aria-describedby` al hint. El hint de error **propone la corrección**: "falta el final del dominio, probá vale.mendez@gmail.com", nunca "email inválido".
- **Bloques:** A2, A3, C5, E3, F7, J3, K2.
- **Prohibido:** `type="number"` para plata (es `Keypad`) y `<select>` nativo (es `SegmentedControl` o `Sheet`).

### CurrencyChip
- **Existe para:** la moneda de un movimiento, con la bandera como identidad de país.
- **Props:** `currency?: CurrencyCode` · `selected?: boolean` · `showChevron?: boolean` · `onClick?: () => void`.
- **Estados:** normal · seleccionado (relleno de marca: es identidad de dato) · presionado · sin chevron (display).
- **Tokens:** `--surface-2 --primary-fill --border --font-mono --radius-chip`.
- **A11y:** `aria-label="Moneda: USD"`; la bandera es `aria-hidden`. **El código va siempre al lado de la bandera** — un emoji de bandera no es un nombre de moneda y muchos lectores no lo anuncian.
- **Bloques:** C1, C4, E3, E6, H6, I5.
- **Nota:** único emoji admitido en todo el sistema.

### Sparkline
- **Existe para:** un glifo de tendencia dentro de otro elemento. Ver 8.1.
- **Props:** `values: number[]` · `width?: number` (≤120) · `height?: number` (≤32) · `color?: string`.
- **Estados:** una sola serie. Sin interacción, sin ejes, sin labels, sin tooltip.
- **Tokens:** `--data-1..5 --line-width`.
- **A11y:** `aria-hidden="true"`: el dato ya está en texto en la fila que lo contiene. Si no lo está, el sparkline está mal usado y corresponde `LineChart`.
- **Bloques:** B3, D4, G1, I2, I3 (7 usos que se quedan). Los 2 de F2 migran a `LineChart`.

### CategoryBubble
- **Existe para:** elegir la categoría de un gasto en el paso de clasificación.
- **Props:** `icon?: IconName` · `label: string` · `selected?: boolean` · `onClick?: () => void`.
- **Estados:** normal · seleccionada (superficie 3 + anillo de borde + `scale(1.04)`) · presionada.
- **Tokens:** `--surface-2 --surface-3 --border --text-primary --text-secondary --press-scale`.
- **A11y:** `role="radio"` + `aria-checked` dentro de un `radiogroup`. Burbuja de 64 px: en la pantalla de clasificación **los targets son el contenido**, por eso son grandes.
- **Bloques:** C2, C3, F7, K5.
- **Nota de historia:** la spec original pedía un **anillo violeta animado**. La regla de selección por superficie es posterior y gana: se conserva la animación del anillo, cambia el color. Sin esto, seis burbujas serían seis violetas.

### InsightCard
- **Existe para:** una observación con una acción. Una por pantalla.
- **Props:** `status?: 'neutral'|'good'|'warning'|'serious'|'critical'` · `icon?: IconName` · `text: string` · `actionLabel?: string` · `onAction?: () => void` · `sparkline?: ReactNode` · `onDismiss?: () => void`.
- **Estados:** los cinco niveles · con sparkline · descartable (swipe o cruz).
- **Tokens:** `--surface-1 --radius-card --primary-ink` y los colores de estado.
- **A11y:** `role="note"`; la acción es un botón real en `--primary-ink` (tinta, no relleno: no compite con el primario). Descartar es 44×44 con `aria-label`.
- **Bloques:** B2, D5, F4, G1, H10, J2.
- **Regla:** si hay dos cosas que decir, **se agrupan en una card**, no se apilan dos. "2 categorías excedidas · $ 2.310 arriba".

### SyncDot
- **Existe para:** el estado de sincronización, sin ocupar lugar.
- **Props:** `state?: 'synced'|'syncing'|'offline'` · `pending?: number`.
- **Estados:** sincronizado (gris, 6 px) · sincronizando (pulso 1,2 s) · sin conexión con conteo (warning).
- **Tokens:** `--text-muted --warning --font-mono --ease-spring-soft`.
- **A11y:** `role="status"` con `aria-label` que dice el estado **en palabras** — un punto de 6 px de color no comunica nada por sí solo, y esto es exactamente la regla de "el color nunca porta significado solo".
- **Bloques:** todos (vive en `AppHeader`).

### SeriesLegend
- **Existe para:** identificar series, y para ser la vista de tabla que todo gráfico debe ofrecer.
- **Props:** `series: {label, color?, value?}[]` · `layout?: 'inline'|'table'` · `dividers?: boolean`.
- **Estados:** en línea (bajo el gráfico) · tabla (reemplaza al gráfico).
- **Tokens:** `--data-1..5 --data-other --text-primary --text-secondary --gridline`.
- **A11y:** en `table` usa `role="table"/"row"/"cell"` y es la **alternativa accesible obligatoria** de todo gráfico. El label va en tinta, nunca en el color de la serie.
- **Bloques:** F2, F6, H2–H9, I2, I10, J8.
- **Regla:** obligatoria con dos o más series. Con una, prohibida (es ruido).

### BarChart
- **Existe para:** comparar una magnitud entre categorías o períodos.
- **Props:** `data: {label, value}[]` · `height?: number` · `color?: string` · `gridLines?: number` · `labelExtremes?: boolean` · `inProgressIndex?: number`.
- **Estados:** normal · con período en curso en gris · vacío (no renderiza: la card muestra cuánto falta).
- **Tokens:** `--data-1..5 --gridline --text-muted`.
- **A11y:** `role="img"` con label que enumera los pares. Barras finas con **extremo redondeado de 4 px ancladas a la baseline**; grilla hairline sólo horizontal, sin bordes de caja; labels directos selectivos y **nunca** un número sobre cada barra.
- **Bloques:** F6, G2, G4, H3, H5, H7, H9, I10, I11.
- **Cambio:** `inProgressIndex` es nuevo — el período que todavía no cerró va en gris, y esa regla estaba en las pantallas y no en el componente.

### DateStrip
- **Existe para:** elegir la fecha de un movimiento sin abrir un date picker modal.
- **Props:** `days: (string | {date, label})[]` · `value: string` (ISO) · `onChange: (iso) => void` · `onLongPress?: (iso) => void`.
- **Estados:** día normal · día nombrado ("Hoy", "Ayer", en peso 600 y tinta primaria) · seleccionado (superficie 3) · long-press abre el calendario completo.
- **Tokens:** `--surface-2 --surface-3 --radius-input --text-primary --text-secondary --text-muted --font-mono`.
- **A11y:** `role="radiogroup"`; cada día `role="radio"` + `aria-checked` y `aria-label` con la fecha **completa hablada** ("martes 21 de julio"), no "21". Carril con scroll-snap; celdas de 52×64.
- **Bloques:** C1, C3, G3, G6, H8.
- **Nota:** los días nombrados ganan al día de la semana y no muestran número: "Hoy" es más rápido de leer que "MAR 29".

### FxEditor
- **Existe para:** **escribir** un tipo de cambio. Ver 8.0.
- **Props:** `from?: CurrencyCode` · `to?: CurrencyCode` · `rate: number` · `suggested?: number` · `decimals?: number` · `source?: string` · `ageHours?: number` · `state?: PriceState` · `onChange: (v) => void` · `onOpenKeypad?: () => void`.
- **Estados:** con sugerida (el slider se mueve ±5% alrededor) · sin sugerida (el slider parte del valor actual) · con cotización vieja (compone `PriceStatus state="stale"`) · manual (`state="manual"`) · editando con keypad.
- **Tokens:** `--font-mono --text-hero-* --primary-fill --text-muted`.
- **A11y:** la cifra es un `button` con `aria-live="polite"` que abre el keypad; el slider tiene `aria-label` propio y 44 px de alto. La cifra **en mono**, no en sans: es un número que se compara.
- **Bloques:** C4, K8.
- **Cambios respecto del bundle:** (1) pierde `stale: boolean`, gana `state: PriceState` y compone `PriceStatus`; (2) **la precisión sale del par** — 2 decimales para USD/UYU, **4 para ARS/UYU** — y ya no está hardcodeada en 2; (3) la cifra pasa de `--font-sans` a `--font-mono`.

### AmountScrubber
- **Existe para:** ajustar un monto arrastrando, sin abrir el keypad.
- **Props:** `value: number` · `currency?: CurrencyCode` · `step?: number` · `min?: number` · `size?: AmountSize` · `onChange: (v) => void` · `onOpenKeypad?: () => void`.
- **Estados:** normal · arrastrando (`scale(1.02)`) · tap corto (<180 ms) abre el keypad · en el mínimo.
- **Tokens:** los de `Amount` más `--duration-fast --ease-spring-snappy`.
- **A11y:** `role="slider"` con `aria-valuenow`, `aria-live="polite"` y flechas del teclado moviendo un `step`. `touch-action: none` para que el arrastre no scrollee la página.
- **Bloques:** C1, F6, G3, I5, I6.
- **Nota de diseño:** la **aceleración por velocidad** (hasta ×6) es lo que hace que $ 10 y $ 40.000 estén a la misma distancia del pulgar; sin ella el scrubber sólo sirve para ajustes chicos. Es el mismo gesto físico que `RadialDial` y por eso ambos comparten el haptic cada N pasos.
- **Cambio:** gana `min`, `size` y navegación por teclado; antes no tenía ninguna de las tres y no era operable sin puntero.

---

## 9 · Qué del bundle NO entró en la librería, y por qué

- **ScopeSwitcher** — eliminado, no portado. Es `SegmentedControl emphasis="brand"` dentro de `AppHeader`. Dejar un alias garantiza que aparezca un segundo control de alcance.
- **`formatAmount` como export del bundle** — sigue existiendo, pero ahora `decimals` es derivable y `formatNumber` **exige** el suyo. El `formatAmount(v, c)` de dos argumentos queda para el caso trivial.
- **Los `.d.ts` y `.prompt.md` por componente** — no se replican en el archivo único: este contrato los reemplaza y el port a TypeScript genera los tipos desde el código.
- **`ui_kits/app/`** — es una recreación click-through, no biblioteca.
- **Nada de `guidelines/`** — son fichas de fundamentos, ya destiladas en la sección 0.

## 10 · Cuáles de los veinte exigieron decidir, no documentar

Seis. Los otros catorce fueron transcripción del comportamiento existente.

1. **FxEditor** — la decisión grande: tres componentes con un átomo compartido, no uno que absorbe a los otros. Y de paso salió un bug: 2 decimales hardcodeados rompen ARS/UYU.
2. **Sparkline** — decidir que se queda con alcance recortado en vez de migrar los nueve usos. Dos migran, siete no, y la regla que los separa es de contenido (una serie vs. dos), no de tamaño.
3. **Amount** — `count` es una prop nueva que no existía en ninguna parte. La alternativa era un `Count` aparte; preferí la prop porque la tipografía y el `tabular-nums` son los mismos y un componente nuevo se iba a olvidar.
4. **Chip** — hubo que **escribir la frontera** entre relleno de marca y superficie ("filtro activo" vs. "elección de configuración"). No estaba en ningún lado y produjo dos tratamientos opuestos entre D2 e I1.
5. **Switch** — la excepción de violeta estaba declarada para una pantalla y en la práctica corre en catorce. Hubo que reescribirla como regla general con su condición ("una pantalla de switches no lleva primario") y admitir la sub-excepción de las pantallas de activación.
6. **CategoryBubble** — resolver un conflicto explícito con la especificación original, que pedía anillo **violeta**. Gana la regla de selección por superficie; se conserva la animación.

Y dos correcciones que no son de estos veinte pero van adentro de la librería porque el contrato ya las pedía: **EmptyState** ahora usa `ZMark` (eran dos sistemas de vacío conviviendo, con 68 pantallas del lado equivocado) y **SplitBar** dejó de pintar con la paleta de datos, que `charts.css` prohíbe textualmente.

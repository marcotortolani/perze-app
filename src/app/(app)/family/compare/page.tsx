...
export default function ComparePage() {
  ...
  const categoriesData = useMemo(() => {
    if (!household || !members || !transactions || !sharesQuery.data || !userId) return [];
    ...
    return [...byCategory.entries()].map(([catId, perMember]) => ({
      label: catId === "__none" ? t("transactions.detail.noCategory") : categoryLabel(catId),
  ...
  }, [household, members, transactions, sharesQuery.data, userId]);

  if (!household || !members || !transactions || !userId || sharesQuery.isLoading) return <Skeleton height={260} style={{ marginTop: 16 }} />;

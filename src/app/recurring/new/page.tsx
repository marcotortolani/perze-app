...
import { Button, IconButton, Input, Keypad, KeypadKey, ListRow, SegmentedControl, Sheet, Switch } from "@/design-system";
...
/**
 * G3 — nueva regla recurrente. Ruta de página completa, mismo patrón que
 * `/add` y `accounts/new`: el diseño (`bloque-fg-presupuestos.html`,
 * las cuatro variantes de estado de G3) lleva `tabs="{{ false }}"` — nunca
 * tuvo tab bar. Vivía por error adentro de `(app)/`, heredando una tab bar
 * y un FAB que el diseño no pedía; de ahí que el botón "Guardar" quedara
 * tapado por el "+" en mobile sin importar cuánto padding se le agregara
 * — se estaba compensando algo que no debía estar ahí en primer lugar.
 */
  ...
  return (
    <ScreenShell style={{ padding: "16px var(--screen-padding) calc(var(--screen-padding) + env(safe-area-inset-bottom))", gap: 16 }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <IconButton icon="chevron-left" ariaLabel={t("ds.appHeader.back")} onClick={() => router.back()} style={{ margin: -11 }} />
      ...
      <div className="flex flex-col gap-4">
        ...
        {isDayBased ? (
          <div className="flex flex-row gap-4">
            {/* Los nombres de mes ("septiembre") pesan bastante más que el
                día (1-2 dígitos) — 3:2 en vez de 1:1. Mismo look que
                `Input` (label afuera, campo de 48px con
      ...
      {/* "=" + "Guardar" comparten fila, igual que `AmountStep.tsx` en `/add`: 1:3 en reposo, 2:2 mientras hay un operador pendiente. */}

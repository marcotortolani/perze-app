...
describe("computePositions", () => {
  it("accumulates quantity and cost basis across buys", () => {
    const positions = computePositions([
      { id: "1", instrumentId: "AAPL", kind: "buy", quantity: 10, netAmount: 1000n, executedAt: "2026-01-01T00:00:00Z" },
      { id: "2", instrumentId: "AAPL", kind: "buy", quantity: 5, netAmount: 600n, executedAt: "2026-01-02T00:00:00Z" },
    ]);
  ...
  it("reduces quantity and cost basis proportionally on a partial sell", () => {
    const positions = computePositions([
      { id: "1", instrumentId: "AAPL", kind: "buy", quantity: 10, netAmount: 1000n, executedAt: "2026-01-01T00:00:00Z" },
      { id: "2", instrumentId: "AAPL", kind: "sell", quantity: 5, netAmount: 550n, executedAt: "2026-01-02T00:00:00Z" },
    ]);
    const pos = positions.get("AAPL")!;
  ...
  it("closes a position entirely sold out — it doesn't linger at 0", () => {
    const positions = computePositions([
      { id: "1", instrumentId: "AAPL", kind: "buy", quantity: 10, netAmount: 1000n, executedAt: "2026-01-01T00:00:00Z" },
      { id: "2", instrumentId: "AAPL", kind: "sell", quantity: 10, netAmount: 1200n, executedAt: "2026-01-02T00:00:00Z" },
    ]);
  ...
  it("ignores non-quantity trade kinds like dividends and fees", () => {
    const positions = computePositions([
      { id: "1", instrumentId: "AAPL", kind: "buy", quantity: 10, netAmount: 1000n, executedAt: "2026-01-01T00:00:00Z" },
      { id: "2", instrumentId: "AAPL", kind: "dividend", quantity: 0, netAmount: 50n, executedAt: "2026-01-02T00:00:00Z" },
      { id: "3", instrumentId: "AAPL", kind: "fee", quantity: 0, netAmount: -10n, executedAt: "2026-01-03T00:00:00Z" },
    ]);
    const pos = positions.get("AAPL")!;
  ...
  it("tracks multiple instruments independently", () => {
    const positions = computePositions([
      { id: "1", instrumentId: "AAPL", kind: "buy", quantity: 10, netAmount: 1000n, executedAt: "2026-01-01T00:00:00Z" },
      { id: "2", instrumentId: "BTC", kind: "buy", quantity: 0.5, netAmount: 2000n, executedAt: "2026-01-01T00:00:00Z" },
    ]);
  ...
  it("transfer_in (posición inicial) suma cantidad y costo base igual que una compra", () => {
    const positions = computePositions([{ id: "1", instrumentId: "SPCX", kind: "transfer_in", quantity: 10, netAmount: 1000n, executedAt: "2026-01-01T00:00:00Z" }]);
  ...
  it("una compra posterior se suma sobre la posición inicial cargada con transfer_in", () => {
    const positions = computePositions([
      { id: "1", instrumentId: "SPCX", kind: "transfer_in", quantity: 10, netAmount: 1000n, executedAt: "2026-01-01T00:00:00Z" },
      { id: "2", instrumentId: "SPCX", kind: "buy", quantity: 5, netAmount: 600n, executedAt: "2026-01-02T00:00:00Z" },
    ]);
    const pos = positions.get("SPCX")!;
    expect(pos.quantity).toBe(15);
    expect(pos.costBasis).toBe(1600n);
  ...
  it("transfer_out reduce cantidad y costo base proporcionalmente, igual que una venta", () => {
    const positions = computePositions([
      { id: "1", instrumentId: "SPCX", kind: "transfer_in", quantity: 10, netAmount: 1000n, executedAt: "2026-01-01T00:00:00Z" },
      { id: "2", instrumentId: "SPCX", kind: "transfer_out", quantity: 5, netAmount: 0n, executedAt: "2026-01-02T00:00:00Z" },
    ]);
    const pos = positions.get("SPCX")!;
    expect(pos.quantity).toBe(5);
  ...
  it("transfer_in + venta total cierra la posición — no queda en 0 para siempre", () => {
    const positions = computePositions([
      { id: "1", instrumentId: "SPCX", kind: "transfer_in", quantity: 10, netAmount: 1000n, executedAt: "2026-01-01T00:00:00Z" },
      { id: "2", instrumentId: "SPCX", kind: "sell", quantity: 10, netAmount: 1200n, executedAt: "2026-01-02T00:00:00Z" },
    ]);
  ...
  it("revaluation no mueve cantidad ni costo base", () => {
    const positions = computePositions([
      { id: "1", instrumentId: "SPCX", kind: "transfer_in", quantity: 10, netAmount: 1000n, executedAt: "2026-01-01T00:00:00Z" },
      { id: "2", instrumentId: "SPCX", kind: "revaluation", quantity: 0, netAmount: 500n, executedAt: "2026-01-02T00:00:00Z" },
    ]);
    const pos = positions.get("SPCX")!;
    expect(pos.quantity).toBe(10);
    expect(pos.costBasis).toBe(1000n);

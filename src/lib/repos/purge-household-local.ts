...
export async function wipeLocalHouseholdData(householdId: string): Promise<void> {
  ...
  // Tanda 4 — grupo de tarjeta multi-moneda: si no se limpia acá, un purge
  // deja filas de `account_groups` huérfanas (sin ninguna cuenta que las
  // referencie) y su entrada de outbox puede resucitarlas en el servidor.
  const accountGroupIds = (await db.accountGroups.where("householdId").equals(householdId).primaryKeys()) as string[];
  ...
  await Promise.all([
    ...
    db.accountGroups.where("householdId").equals(householdId).delete(),
  ...
  await wipeOutboxFor({
    transactions: txIds,
    accounts: accountIds,
    account_groups: accountGroupIds,
  ...
  const conflictEntityIds = [...txIds, ...accountIds, ...accountGroupIds, ...categoryIds, ...tagIds, ...payeeIds, ...budgetIds, ...goalIds, ...recurringRuleIds, ...categorizationRuleIds];

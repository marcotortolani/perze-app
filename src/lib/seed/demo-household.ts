...
import { withoutOutbox } from "../offline/outbox";
...
export async function seedDemoHousehold(): Promise<{ householdId: string }> {
  // B15 — el demo no debe intentar sincronizar nunca: `USER_ID` es un uuid
  // fijo que ninguna policy de RLS reconoce, así que cualquier enqueue acá
  // se quedaría en el outbox para siempre.
  return withoutOutbox(async () => {
